...
public class Agent {
	...
	public enum Level01GoalState {FIND_GOLD, CHART_MAP};
	public enum Level02GoalState {EXPLORE_NEIGHBORHOOD, PLAN_TRAVEL, TRAVEL_TO_SAFE, TRAVEL_TO_BEST_WUMPUS, TEST_DUNGEON,CHART_AXIS};
	...
	private String createGame() throws IOException {
		String id = post("{email:eff.fay@gmail.com, seed:"+seed+"}",new URL("http://54.207.125.93:8080/wumpus/creategame"));
		CreateGamePackage pack = new Gson().fromJson(id, CreateGamePackage.class);
		logger.info("ID = {}",pack.id);
		return pack.id;
	}
		
	private Percept percept() throws IOException {
