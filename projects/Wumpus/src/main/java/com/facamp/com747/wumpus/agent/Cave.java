...
public class Cave {
	...
	public TravelPlan findNearestMaybePitDungeon(int x, int y) {
        ...
        /*public TravelPlan findNearestEdge(int x, int y, State desiredWumpusState, boolean considerWumpusAsThreat) {
		List<Node> fringe;
		List<Node> closed;
		fringe = new ArrayList<>();
		closed = new ArrayList<>();
		...
		while(!fringe.isEmpty()) {
			...
			my = n.d.y - minKnownHorizonVertical;
			closed.add(n);
			if (n.d.getWumpus() == desiredWumpusState && n.d.getPit() == State.NO && !(n.d.getX()==x && n.d.getY()==y)) {
				ArrayList<Dungeon> ld = new ArrayList<>();
				while(n!=null) {
					if (n.father!=null)
						ld.add(0,n.d);
